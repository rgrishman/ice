...
public class TokenTypeFeatureExtractor implements RelationFeatureExtractor {
    public Event extractFeatures(AceEntityMention m1,
        ...
        List<String> feats = TokenFeatureExtractor.extractSameTokens(h1Start, h2End, doc);
        ...
        for (String feat : feats) {
            updatedFeats.add(feat);
            updatedFeats.add("CONJ=" + feat + ":::" + concatTypes);
        }
        if (updatedFeats.size() == 0) {
            updatedFeats.add("TOKEN=NONE");
            updatedFeats.add("CONJ=TOKEN=NONE:::" + concatTypes);
        }
        //updatedFeats.add("PATH_LENGTH=" + pathLength);
        updatedFeats.add(type1);
        updatedFeats.add(type2);
        updatedFeats.add(concatTypes);
