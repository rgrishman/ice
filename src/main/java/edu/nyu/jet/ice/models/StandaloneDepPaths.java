...
public class StandaloneDepPaths {

    final static Logger logger = LoggerFactory.getLogger(StandaloneDepPaths.class);
...
//                    if (!sentences.inSameSentence(h1, h2)) continue;
    ...
    public static void main (String[] args) throws IOException {
		...
		String selectedCorpusName = docList.split("\\.")[0];
		System.err.println(String.format("docList:%s => corpusName:%s",
				docList, selectedCorpusName));

		// initialize Jet

		System.err.println("Starting Jet DepCounter ...");
		JetTest.initializeFromConfig(propsFile);
...
//			Ace.buildAceEntities (doc, docId, aceDoc);
			// ---------------
			// invoke parser on 'doc' and accumulate counts
			String cacheDir = FileNameSchema.getPreprocessCacheDir(selectedCorpusName);
