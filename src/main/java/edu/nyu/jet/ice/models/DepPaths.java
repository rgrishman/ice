...
public class DepPaths {
    ...
    /**
     *	Collects all dependency paths between entity mentions in a set
     *	of files.  Invoked by <br>
     *	DepPaths  propsFile docList inputDir inputSuffix outputFile typeOutputFile sourceDictFile <br>
     *
...
//		      if (!sentences.inSameSentence(h1, h2)) continue;
    ...
    /**
     *  recordds dependency path 'path' with associated information (sample sentennce,
     *  linearized form).
     */
    ...
    /**
     *	returns the dependency path from 'fromPosn' to 'toPosn'r, or 'null'
     *  if no such path exisrts. The path is not allowed if one of localMentions 
     *  is on the path (but not at the beginning or end of the path)
     */

    public static DepPath buildSyntacticPathOnSpans (int fromPosn, int toPosn,
     SyntacticRelationSet relations, List<Span> localSpans) {
    ...
    /**
     *  generate text of 'sentence', with 'mention1' and 'mention2' marked by
     *  brackets for use as tooltip.
     */
